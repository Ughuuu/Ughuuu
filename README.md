### Hi there 👋. Here are some of my projects

- [Godot Rapier Physics](https://github.com/appsinacup/godot-rapier-physics)

![Godot Rapier](https://github.com/appsinacup/godot-rapier-physics/blob/main/logo.jpg?raw=true)

A 2D and 3D drop-in replacement for the Godot engine that adds stability and fluids.

Note: This uses the excelent [Rapier](http://rapier.rs) library inside, made by [Dimforge](https://github.com/sponsors/dimforge).

- [Godot SoftBody2D](https://github.com/appsinacup/godot-softbody2d)

![Godot SoftBody2D](https://github.com/appsinacup/godot-softbody2d/blob/main/softbody2d_banner.jpg?raw=true)

Plugin that adds deformable 2D soft body.

- [Godot Whisper](https://github.com/appsinacup/godot-whisper)

![Godot Whisper](https://github.com/appsinacup/godot-whisper/blob/main/banner_godot_whisper.jpg?raw=true)

Plugin that adds speech to text (STT).

- [Godot VSCode IDE](https://github.com/appsinacup/godot_vscode_ide)

![Godot VSCode IDE](https://github.com/appsinacup/godot_vscode_ide/blob/main/docs/example.png?raw=true)

Plugin that adds VSCode inside Godot.

- [Gamend Game Server](https://github.com/appsinacup/game_server)

![Godot Game Server](https://github.com/appsinacup/game_server/blob/main/apps/game_server_host/priv/static/images/banner.png?raw=true)

Game server with authentication, users, lobbies, groups, notifications, server scripting and an admin portal.
